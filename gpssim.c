...
void computeRange(range_t *rho, ephem_t eph, gpstime_t g, const double xyz[])
{
...
void *gps_task(void *arg)
{
	...
	strcpy(umfile, "circle.csv");
	numd = readUserMotion(xyz, umfile);
	if (numd==-1) {
		fprintf(stderr, "Failed to open user motion file.\n");
		goto exit;
	}
	...
	strcpy(navfile, "brdc2130.15n");
	neph = readRinexNav(eph, navfile);

	if (neph==-1) {
		fprintf(stderr, "Failed to open ephemeris file.\n");
		goto exit;
	}
	...
	// Allocate I/Q buffer
	iq_buff = (short *)calloc(NUM_IQ_SAMPLES * 2, sizeof(short));

	if (iq_buff == NULL) {
		fprintf(stderr, "Faild to allocate IQ buffer.\n");
		goto exit;
	}
	...
	printf("Itinitializing channels...\n");
	...
	for (i=0; i<nsat; i++)
	{
		...
		// Allocate I/Q buffer
		chan[i].iq_buff = (short *)calloc(NUM_IQ_SAMPLES * 2, sizeof(short));
	...
	for (iumd=1; iumd<numd; iumd++) {
		...
		for (isamp=0; isamp<(NUM_IQ_SAMPLES * 2); isamp++) {
			short sample = 0;
			for (i=0; i<nsat; i++)
